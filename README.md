...
simple-cipher は Xorlshift とバーナム暗号を組み合わせたシンプルな暗号機能を提供します。
...
```bash
gcc -Idist/include -Ldist/lib example.c -lsimplecipher
```

## Command 

ビルド後に作成される `bin/simple-cipher.exe` を実行することで、コマンドラインから暗号機能を利用することができます。

```bash
./bin/simple-cipher -k 0x123
```
...
simple-cipher released under the [MIT License](./LICENSE).
